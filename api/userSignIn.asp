    ...
    Set rs = ba.Execute(sqll)
        if Not rs.eof Then
            if rs("success") = True Then
                ...
                Results.add "profileId", rs("profileId").value
                Results.add "role", rs("role").value
                if role = "STU" Then
                    Results.add "profile", ba.Execute("SELECT TOP 1 * FROM [StudentDetails] WHERE id='" & rs("profileId").value & "' ")
                Elseif role = "ADM" Or role = "LEC" Then
                    Results.add "profile", ba.Execute("SELECT TOP 1 * FROM [LecturerDetails] WHERE id='" & rs("profileId").value & "' ")
                End if
            Else 
                jSuccess =  false
